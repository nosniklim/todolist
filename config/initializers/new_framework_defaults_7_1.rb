...
#++
# NOTE: ローカル環境ではファイル容量が問題になるケースは稀でログローテートするほどではないが有効化
if Rails.env.local?
...
#++
# NOTE: カラムにserializeメソッドを使用している場合のデフォルトがYAMLからJSONに変更
...
#++
# NOTE: モデル内の定義順にafter_commitコールバックが実行されるようになった
...
#++
# NOTE: デフォルトのログレベルの推奨は :error（:debug / :info / :warn / :error / :fatal）
